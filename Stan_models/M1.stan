...
transformed parameters {
  ...
  for(i in 1:Ntot){
    pred_log10_vl[i] =
    alpha_0 + theta_rand_id[id[i]][1] + theta_epoch_prime[1][epoch[i]] + gamma_rnasep*RNaseP[i] +
    beta_0*exp(trt_effect_prime[trt[i]]+theta_rand_id[id[i]][2]+theta_epoch_prime[2][epoch[i]])*obs_day[i];
  }
}
