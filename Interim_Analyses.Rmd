...
for(dd in names(analysis_datasets)[-1]){
  trt_cols = brewer.pal(n = 9,
                        name = 'Paired')
  plot_serial_data(xx = platcov_dat_list[[dd]],
                   Trt = platcov_dat_list[[dd]]$Trt,
                   trt_cols = trt_cols)
}
```
...
par(las=1, mar=c(5, 12, 2,2))
for(dd in analysis_datasets){
  ind = which(model_settings$dataset==dd & model_settings$prior==1)
...
```{r treatment_effects}
for(i in which(model_settings$dataset != 'REGN_analysis.csv')){
...
```{r}
hist(platcov_dat_list[['Favipiravir_analysis.csv']]$Weight[ind_fitting_list[['Favipiravir_analysis.csv']] & !duplicated(platcov_dat_list[['Favipiravir_analysis.csv']]$ID)],main='',xlab='kg')

ind_start = stan_inputs[['Favipiravir_analysis.csv']]$analysis_data_stan$ind_start
ids_stan_fvp = stan_inputs[['Favipiravir_analysis.csv']]$analysis_data_stan$id[ind_start]
ids_true_fvp = 
  plyr::mapvalues(ids_stan_fvp, 
                  from = stan_inputs[['Favipiravir_analysis.csv']]$ID_map$ID_stan,
                  to = stan_inputs[['Favipiravir_analysis.csv']]$ID_map$ID_key)

ind_dup = !duplicated(platcov_dat_list[['Favipiravir_analysis.csv']]$ID)
trt_allocations_fvp =
  plyr::mapvalues(ids_true_fvp, 
                  from=platcov_dat_list[['Favipiravir_analysis.csv']]$ID[ind_dup],
                  to=as.character(platcov_dat_list[['Favipiravir_analysis.csv']]$Trt[ind_dup]))

weight_kg_fvp = 
  as.numeric(plyr::mapvalues(ids_true_fvp, 
                             from = platcov_dat_list[['Favipiravir_analysis.csv']]$ID[ind_dup],
                             to = platcov_dat_list[['Favipiravir_analysis.csv']]$Weight[ind_dup]))
...
fvp_summary = data.frame(ID=ids_true_fvp, 
                         slope=slopes_fvp, 
                         dose_mg_kg=1/weight_kg_fvp,
                         Trt=trt_allocations_fvp)
fvp_summary$dose_mg_kg[fvp_summary$Trt=='No study drug']=0

summary(lm(slope ~ dose_mg_kg, data = fvp_summary[fvp_summary$Trt=='Favipiravir',]))
...
# ID_map = stan_inputs[['interim_dat.csv']]$ID_map
# ind_not_dup = which(!duplicated(platcov_dat_list[["interim_dat.csv"]]$ID))
# ID_map$Trt = 
#   plyr::mapvalues(ID_map$ID_key, 
#                   from = platcov_dat_list[["interim_dat.csv"]]$ID[ind_not_dup], 
#                   to = as.character(platcov_dat_list[["interim_dat.csv"]]$Trt[ind_not_dup]))
# out_all = list()
...
#     thetas[[mm]] = rstan::extract(out_all[[mm]])
#   }
# 
# 
# for(my_trt in unique(ID_map$Trt)){
...
#                   ID_map = ID_map[which(ID_map$Trt==my_trt), ],
#                   analysis_data_stan = stan_inputs[['interim_dat.csv']]$analysis_data_stan)
# }
```
