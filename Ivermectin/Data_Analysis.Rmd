---
title: "PLATCOV Statistical Analysis: Ivermectin Results"
author: "James Watson"
...
This analysis is based on the prespecified Statistical Analysis Plan (SAP) of the PLATCOV trial (registered at ClinicalTrials.gov: https://clinicaltrials.gov/ct2/show/NCT05041907).
This file does the data anlysis for the Ivermectin arm (and concurrent controls).
...
table(platcov_dat$Trt[!duplicated(platcov_dat$ID)])
...
par(las=1, cex.lab=1.5, cex.axis=1.5, family='serif', mfrow=c(1,2),bty='n')
...
PCR_dat = aggregate(log10_viral_load_pred ~ ID + Timepoint_ID + Trt + Site + Trt_number, 
                    data = platcov_dat,
...
trt_smmry = aggregate(formula = log10_viral_load_pred ~ Timepoint_ID+Trt_number, 
                      data = platcov_dat, FUN = median)

plot(PCR_dat$Timepoint_ID, PCR_dat$log10_viral_load_pred[,'mean'],
     ylab = 'Viral copies per mL', panel.first=grid(),
     xlab = 'Time since randomisation (days)',
     xlim = c(0,7), type='n', yaxt='n',
     col = trt_cols[PCR_dat$Trt_number],
     ylim = c(1, max(PCR_dat$log10_viral_load_pred[,'mean'])))
...
for(id in IDs){
  ind = PCR_dat$ID==id
  lines(PCR_dat$Timepoint_ID[ind],
        PCR_dat$log10_viral_load_pred[ind,'mean'],
...
X_matrix = model.matrix( ~ Epoch + Age_scaled + Antibody_test +
...
nrow(X_matrix) == nrow(platcov_dat_fit)
```
...
# extract parameters
rsqds = array(dim = c(Nchain*(Niter-Nwarmup)/Nthin, K_models))
thetas = list()
for(i in 1:K_models){
...
effect_ests_het = effect_ests = list()
for(mm in 1:K_models){
...
mod_cols = brewer.pal(n = K_models,name = 'Set1')
par(bty='n', cex.lab=1.5, cex.axis=1.5,family='serif',
    mar=c(5,7,2,2),las=1, mfrow=c(1,1))
xlims = range(sapply(effect_ests, rbind))
plot(NA, NA, xlim = xlims, ylim = c(0.5,length(trts)+.5), 
...
alpha_coefs = apply(thetas[[3]]$intercept_coefs,2,
                    quantile,probs=c(0.025,.1,.5,.9,0.975))
xlims=range(alpha_coefs)

cov_names = plyr::mapvalues(x = colnames(X_matrix), 
...
axis(2, at =1:ncol(alpha_coefs), labels = cov_names,tick = F)
...
axis(2, at =1:ncol(beta_coefs), labels = cov_names, tick = F)
x_points = round(10^seq(xlims[1], xlims[2],length.out = 5),1)
axis(1, at = log10(x_points), labels = x_points)
```
...
for(tt in c('No study drug', trts)){
  ...
  if(tt == 'Regeneron'){
    text(x = as.POSIXct('2022-02-01'),y = 1.4,
         labels = paste0(tt,' (n=',length(ind),')'),cex=2)
  } else {
    text(x = as.POSIXct('2021-10-20'),y = 1.4,
         labels = paste0(tt,' (n=',length(ind),')'),cex=2)
  }
...
```{r slopes_by_group}
slopes = abs(extract(out[[3]])$slope)
trt_summary_dat = 
  data.frame(trt = as.character(platcov_dat_fit$Trt_code[analysis_data_stan$ind_start]),
             epoch = analysis_data_stan$epoch[analysis_data_stan$ind_start],
