...
plot(platcov_dat$Rand_date[ind_dup], 1:length(IDs),
     ...
     xlab='Randomisation date', ylab='Patient ID',panel.first=grid())
...
save(model_settings, stan_inputs, all_priors,
...
writeLines(sprintf('Under model %s the treatment effect for ivermectin is %s (95%% %s to %s)',
                   model_report,
                   round(exp(mean(thetas_main_model$trt_effect[,2])),2),
                   round(exp(quantile(thetas_main_model$trt_effect[,2],0.025)),2),
                   round(exp(quantile(thetas_main_model$trt_effect[,2],0.975)),2)))

writeLines(sprintf('Under model %s the treatment effect for Regeneron is %s (95%% %s to %s)',
                   model_report,
                   round(exp(mean(thetas_main_model$trt_effect[,1])),2),
                   round(exp(quantile(thetas_main_model$trt_effect[,1],0.025)),2),
                   round(exp(quantile(thetas_main_model$trt_effect[,1],0.975)),2)))
