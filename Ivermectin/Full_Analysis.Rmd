...
Dmax = 8
RUN_MODELS = F
source('../functions.R') # some useful plotting functions
```
...
PCR_dat = aggregate(log10_viral_load_pred ~ ID + Timepoint_ID + Trt_number + Trt, 
...
for(id in IDs){
  ...
  gap.plot(PCR_dat$Timepoint_ID[ind],
        PCR_dat$log10_viral_load_pred[ind],add = T,
        gap = c(7.5,13.5), gap.axis = 'x',type='b',
        col=adjustcolor(trt_cols[PCR_dat$Trt_number[ind]],.3))
}
points(trt_smmry$Timepoint_ID, trt_smmry$log10_viral_load_pred,
...
for(tt in unique(trt_smmry$Trt_number)){
  ...
  lines(trt_smmry$Timepoint_ID[ind],
...
writeLines(sprintf('We are running the %s models with %s chains and %s samples for each chain, discarding half for burn-in and thining every %s, thus giving a total of %s posterior samples per model.',
                   length(all_mods),Nchain,Niter,Nthin, 
...
coef_mods = grep('M2.stan',x = model_settings$mod)[1]
...
```{r slopes_over_time}
slope_model = which(model_settings$mod=='../Stan_models/M1.stan'&
...
```{r slopes_by_group}
slopes = abs(extract(out[[2]])$slope)
