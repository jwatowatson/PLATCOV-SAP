...
writeLines(sprintf('In the %s patients, the geometric mean baseline (defined as samples taken within 6 hours of randomisation) viral load was %s copies per mL (interquartile range from %s to %s; range from %s to %s)',
                   length(IDs),
                   round(10^mean(bvl$log10_viral_load)),
                   round(quantile(10^(bvl$log10_viral_load), 0.25)),
                   round(quantile(10^(bvl$log10_viral_load), 0.75)),
...
writeLines(sprintf('There are %s RNaseP CT values.', nrow(platcov_dat)))
...
X_intcpt_1 = model.matrix( ~ Epoch + Site, 
                           data = platcov_dat_fit)[, -1, drop=F]
X_intcpt_2 = model.matrix( ~ Epoch + Site + Age_scaled + Antibody_test + Symptom_onset + N_dose, 
                           data = platcov_dat_fit)[, -1, drop=F]
X_slope_1 = model.matrix( ~ Epoch + Site, 
                          data = platcov_dat_fit)[, -1, drop=F]
X_slope_2 = model.matrix( ~ Epoch + Site + Age_scaled + Antibody_test + Symptom_onset + N_dose, 
                          data = platcov_dat_fit)[, -1, drop=F]
...
main_models_sens = which((model_settings$prior==2 & model_settings$cov_matrices==1) |
                           (model_settings$prior==2 & 
                              model_settings$cov_matrices==2 &
                              model_settings$mod==all_mods[2]))

writeLines('The main 3 models with non-informative priors:')
plot_effect_estimates(effect_ests = effect_ests,
                      plot_models = main_models_sens,
                      names_plot_models = c('Linear: RNaseP',
                                            'Non-linear',
                                            'Linear: covariate'))
