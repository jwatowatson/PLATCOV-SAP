...
# model.matrix( ~ Epoch + Age_scaled + Antibody_test + Symptom_onset + N_dose,
# data = platcov_dat_fit)[, -1]
X_matrix = model.matrix( ~ Epoch + Age_scaled + Antibody_test + Symptom_onset + N_dose, 
                         data = platcov_dat_fit)[, -1]
...
par(bty='n', cex.lab=1.5, cex.axis=1.5,family='serif',
    mar=c(5,7,2,2),las=1, mfrow=c(1,2))
xlims = range(range(sapply(effect_ests, rbind)), 
              range(sapply(effect_ests_het, rbind)))
...
plot(NA, NA, xlim = xlims, ylim = c(0.5,length(trts_var)+.5), 
     panel.first=grid(), ylab='', yaxt='n', type='n',
     xlab = 'Slope (multiplicative effect)', xaxt='n')
axis(1, at = log(c(0.7,1,1.5,2)), labels = c(0.7,1,1.5,2))
...
legend('right', col=mod_cols,lwd = 2, 
       title = 'Model',cex=.8,bty='n',
       legend = c('1 (vanilla)',
                  '2 (RNaseP)',
                  '3 (covariates)',
                  '4 (non-linear)'))
