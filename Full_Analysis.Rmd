...
* Load randomisation data files and cross check
...
for(mm in 1:nrow(model_settings)){
  ...
  if(model_settings$subgroup[mm]==0){ my_trt_names = trts } else {my_trt_names = trts_subgroup}
  names(trt_estimate[[mm]]) = my_trt_names
  names(prob_superior[[mm]]) = my_trt_names
  rownames(effect_ests[[mm]]) = my_trt_names
}


writeLines('\n*******************\nMean estimated treatment effects (multiplicative):')
print(do.call(rbind, trt_estimate[model_settings$subgroup==0]))
print(do.call(rbind, trt_estimate[model_settings$subgroup==1]))

writeLines('\n*******************\nProbability of super-superiority:')
print(do.call(rbind, prob_superior[model_settings$subgroup==0]))
print(do.call(rbind, prob_superior[model_settings$subgroup==1]))
```
...
main_models = which(model_settings$prior==1 & model_settings$subgroup==0)
...
plot_effect_estimates(effect_ests = effect_ests,
                      ...
                      names_plot_models = main_models,
                      study_threshold = study_threshold)
model_report = main_models[1]
main_models_sg = which(model_settings$prior==1 & model_settings$subgroup==1)
main_models_sg = main_models_sg[!main_models_sg==15]
writeLines('The main 4 models with weakly informative priors:')
plot_effect_estimates(effect_ests = effect_ests,
                      plot_models = main_models_sg,
                      names_plot_models = main_models_sg,
                      study_threshold = study_threshold)
```
...
thetas = extract(out_all[[coef_mods[1]]])
alpha_coefs = apply(thetas$intercept_coefs,2,
                    quantile,probs=c(0.025,.1,.5,.9,0.975))
xlims=range(alpha_coefs)

cov_names_intercept =
  plyr::mapvalues(x = colnames(stan_inputs_overall$cov_matrices$X_int[[2]]),
...
cov_names_slope =
  plyr::mapvalues(x = colnames(stan_inputs_overall$cov_matrices$X_slope[[2]]),
                  from = c('Age_scaled','Antibody_test',
...
hist(apply(log10(0.5)/slopes,2,median),
     main='Median',xlab='t_1/2 (days)')

hist(apply(log10(0.5)/slopes,2,quantile,.9),
     main='Upper 90th percentile',xlab='t_1/2 (days)')
...
for(tt in c('No study drug', trts_comp)){
  ...
  abline(#v = as.POSIXct('2021-12-17'),
    h=mean(t_12_med[controls_ind]),lty=2)
...
abline(v = quantile(trt_summary_dat$t_12[trt_summary_dat$trt=='No study drug'],
                    probs = c(0.1,0.5,0.9)), lty=c(2,1,2))
...
mod_cens=lmercens(Surv(log10_viral_load,cens) ~ 1+Time*Trt_code-Trt_code + (1|ID), data = platcov_dat[ind_fitting, ],REML = F,
...
plot_effect_estimates(effect_ests = effect_ests,
                      ...
                      names_plot_models = which(overall_mods),
                      mylty = mylty[which(overall_mods)],
                      study_threshold = study_threshold)

subgroup_mods = model_settings$subgroup==1
plot_effect_estimates(effect_ests = effect_ests,
                      plot_models = which(subgroup_mods),
                      names_plot_models = which(subgroup_mods),
                      mylty = mylty[which(subgroup_mods)],
                      study_threshold = study_threshold)
```
