...
We fit a mixed effects model to the control data to estimate standard curves for each qPCR plate. This includes random slopes and random intercepts for each assay.
...
preds_cens = 
  predict(conv_mod, 
          newdata = data.frame(CT=rep(40,nrow(platcov_dat)),
                               batch=as.factor(platcov_dat$Plate)))
platcov_dat$log10_viral_load_pred = preds_all
platcov_dat$log10_cens_vl = preds_cens

platcov_dat$log10_viral_load_pred[platcov_dat$CT_NS==40]=
  platcov_dat$log10_cens_vl[platcov_dat$CT_NS==40]
...
prop_neg = aggregate(log10_viral_load_pred ~ Timepoint_ID + Trt_number, data=PCR_dat, FUN = function(x) round(100*(mean(x>2))))
PCR_dat$Timepoint_ID=jitter(PCR_dat$Timepoint_ID)

gap.plot(PCR_dat$Timepoint_ID, PCR_dat$log10_viral_load_pred,
         ylab = 'Viral copies per mL', panel.first=grid(),
...
par(mfrow=c(1,1))
plot(prop_neg$Timepoint_ID, prop_neg$log10_viral_load_pred,
     ylim = c(0,100), col=trt_cols[prop_neg$Trt_number],
     panel.first=grid(), pch=16, xlab='Time since randomisation (days)',
     ylab = 'Patients with viral load > 100 copies per mL (%)')
for(tt in unique(prop_neg$Trt_number)){
  ind = prop_neg$Trt_number==tt
  lines(prop_neg$Timepoint_ID[ind],
        prop_neg$log10_viral_load_pred[ind], 
        lwd=3,col=trt_cols[tt])
}
legend('topright', col=trt_cols, 
       legend = unique(PCR_dat$Trt),title = 'Median',
       lwd=2,pch=14+unique(PCR_dat$Trt_number),
...
K_sites = length(unique(platcov_dat$Site))
par(mfrow=c(1,K_sites))
for(ss in unique(bvl$Site)){
  ind = bvl$Site==ss
  hist(bvl$log10_viral_load[ind],
       breaks = seq(1,9,by=1),
       xlab='Baseline viral load (copies per mL)',
       ylab ='Number of patients',xlim=c(1,8.5),
       main=ss, xaxt ='n')
...
print(prior_params_sensitive)

all_priors = list(prior_params, prior_params_sensitive)
```
...
all(!apply(platcov_dat_fit[, c('Epoch','Age_scaled','Antibody_test','Symptom_onset','N_dose')], 2, function(x) any(is.na(x))))

# make the covariate matrix
platcov_dat_fit$Symptom_onset[is.na(platcov_dat_fit$Symptom_onset)]=1
all(!apply(platcov_dat_fit[, c('Epoch','Age_scaled','Antibody_test','Symptom_onset','N_dose')], 2, function(x) any(is.na(x))))

X_matrix_intercept = model.matrix( ~ Epoch + Symptom_onset, 
                                   data = platcov_dat_fit)[, -1, drop=F]
X_matrix_slope = model.matrix( ~ Epoch + Age_scaled, 
                               data = platcov_dat_fit)[, -1, drop=F]

Trt_matrix = model.matrix( ~ Trt_code, data = platcov_dat_fit)
Trt_matrix[,1]=0 # first column is dummy

Trt_matrix_subgroup = model.matrix( ~ Trt_code_subgroup, data = platcov_dat_fit)
Trt_matrix_subgroup[,1]=0 # first column is dummy

Trt_mats = list(Trt_matrix, Trt_matrix_subgroup)
...
analysis_data_stan = list(Ntot = nrow(platcov_dat_fit),
                          ...
                          K_cov_intercept = ncol(X_matrix_intercept),
                          x_intercept = X_matrix_intercept,
                          K_cov_slope = ncol(X_matrix_slope),
                          x_slope = X_matrix_slope)
...
nrow(X_matrix_intercept) == nrow(platcov_dat_fit)
nrow(X_matrix_slope) == nrow(platcov_dat_fit)
```
...
# we run all the models on the cluster
```
...
key_pars_2 = c(key_pars_1, 'gamma_rnasep')
key_pars_3 = c(key_pars_2, 'slope_coefs', 'intercept_coefs')
key_pars_4 = c(key_pars_2, 'tmax_pop')
key_pars = list(key_pars_1, key_pars_2, key_pars_3, key_pars_4)
...
for(i in 1:nrow(model_settings)){
  ...
  load(paste0('Rout/Rout/model_fits_',i,'.RData'))
  out_all[[i]] = out
...
for(mm in 1:nrow(model_settings)){
  ...
  if(model_settings$trt_mat[mm]==1){
    names(trt_estimate[[mm]]) = trts
    names(prob_superior[[mm]]) = trts
    rownames(effect_ests[[mm]]) = trts
  } else {
    names(trt_estimate[[mm]]) = trts_subgroup
    names(prob_superior[[mm]]) = trts_subgroup
    rownames(effect_ests[[mm]]) = trts_subgroup
  }
...
main_models = which(model_settings$prior==1 & model_settings$trt_mat==1)
...
plot_effect_estimates(effect_ests = effect_ests,
                      ...
                      names_plot_models = c('1: vanilla',
                                            '2: linear',
                                            '3: covariate',
                                            '4: non-linear'))

main_models_sens = which(model_settings$prior==2 & model_settings$trt_mat==1)
writeLines('The main 4 models with non-informative priors:')
plot_effect_estimates(effect_ests = effect_ests,
                      plot_models = main_models_sens,
                      names_plot_models = c('1: vanilla',
                                            '2: linear',
                                            '3: covariate',
                                            '4: non-linear'))
...
main_models2 = which(model_settings$prior==1 & model_settings$trt_mat==2)
writeLines('The main 4 models with weakly informative priors:')
plot_effect_estimates(effect_ests = effect_ests,
                      plot_models = main_models2,
                      names_plot_models = c('1: vanilla',
                                            '2: linear',
                                            '3: covariate',
                                            '4: non-linear'))

main_models2_sens = which(model_settings$prior==2 & model_settings$trt_mat==2)
writeLines('The main 4 models with non-informative priors:')
plot_effect_estimates(effect_ests = effect_ests,
                      plot_models = main_models2_sens,
                      names_plot_models = c('1: vanilla',
                                            '2: linear',
                                            '3: covariate',
                                            '4: non-linear'))
...
# alpha_coefs = apply(thetas[[2]]$intercept_coefs,2,
#                     quantile,probs=c(0.025,.1,.5,.9,0.975))
# xlims=range(alpha_coefs)
# 
# cov_names_intercept = 
#   plyr::mapvalues(x = colnames(X_matrix_intercept), 
...
# 
# cov_names_slope = 
#   plyr::mapvalues(x = colnames(X_matrix_slope), 
#                   from = c('Age_scaled','Antibody_test',
#                            'Symptom_onset','N_dose','Epoch'),
#                   to = c('Age','Serology RDT',
#                          'Days since\nsymptom onset',
...
# 
# beta_coefs = apply(thetas[[3]]$slope_coefs,2,quantile,
...
trts_comp = c('Regeneron','Ivermectin','Favipiravir','Remdesivir')
for(tt in c('No study drug', trts_comp)){
  ...
  if(tt == 'Regeneron'){
    text(x = as.POSIXct('2022-02-01'),y = 2,
         labels = paste0(tt,' (n=',length(ind),')'),cex=2)
  } else {
    text(x = as.POSIXct('2021-11-15'),y = 2,
         labels = paste0(tt,' (n=',length(ind),')'),cex=2)
  }
  ...
  abline(v = as.POSIXct('2021-12-17'),
...
```{r slopes_by_group}
slopes = abs(extract(out[[1]])$slope)
...
trt_summary_dat = 
  data.frame(trt = as.character(platcov_dat_fit$Trt_code[ind_unique]),
             ...
             IgG = platcov_dat_fit$IgG[ind_unique],
             IgM = platcov_dat_fit$IgM[ind_unique])
trt_summary_dat$trt = factor(trt_summary_dat$trt, levels=c("No study drug", trts))
...
```{r}
summary(lm(slope_mean ~ trt + IgG, trt_summary_dat))
```
  ...
  if(counter %% K_plots == 0){
    ...
    for(mm in 1:K_models){
      ...
      polygon(x = c(my_xs, rev(my_xs)),
              y = c(apply(thetas[[mm]]$preds[,ind],2,quantile,probs=0.025)[ix],
                    rev(apply(thetas[[mm]]$preds[,ind],2,quantile,probs=0.975)[ix])),
              border = NA, col = adjustcolor(mod_cols[mm],alpha.f = .4))
      lines(my_xs,
