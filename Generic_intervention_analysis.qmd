...
```{r load_data}
intervention = 'Ineffective' # prefix of analysis file
...
plot the no study drug arm

```{r no_study_drug}
platcov_dat_NSD = platcov_dat %>% filter(Trt=='No study drug') %>% 
  distinct(ID, Timepoint_ID, .keep_all = T)

par(las=1, family='serif', cex.axis=1.3, cex.lab=1.3)
plot(platcov_dat_NSD$Time, platcov_dat_NSD$daily_VL, pch='.',
     xlim = c(0,14), panel.first = grid(),
     xlab='Days', ylab = 'SARS CoV2 genomes/mL', yaxt='n')
axis(2, at = c(2,4,6,8), labels = c(expression(10^2),
                                      expression(10^4),
                                      expression(10^6),
                                      expression(10^8)))
for(id in unique(platcov_dat_NSD$ID)){
  xx = platcov_dat_NSD%>%filter(ID==id)
  lines(xx$Time, xx$daily_VL, col = adjustcolor('grey',.3))
}
```
