...
```{r set folder paths for each user}
user <- "Chang" # Change user here
...
ref_arm = 'No study drug'  #'Nirmatrelvir' #
...
f_name = paste0(prefix_analysis_data, 'Analysis_Data/',intervention,'_analysis.csv')
...
model_setup_f_name = paste0(prefix_analysis_data, 'Rout/model_run_setup_',intervention,'.RData')
save(model_settings, 
     platcov_dat_analysis,
     stan_inputs, 
     all_priors,
...
if(RUN_MODELS){
  ...
  save(mod_naive, file = paste0(prefix_analysis_data, 'Rout/naive_mod.RData'))
} else {
  load(paste0(prefix_analysis_data, 'Rout/naive_mod.RData'))
}
...
IDS_slow = which(slope_out$t_12_med>17 & slope_out$Trt=='Ensitrelvir')
slope_out$ID[IDS_slow]
par(mfrow=c(2,2))
plot_individ_curves(platcov_dat = platcov_dat, IDs = slope_out$ID[IDS_slow],xlims = c(0,14))
# model_list = list()
# for(i in 1:length(ff)){
#   load(ff[i])
#   model_list[[i]] = out
